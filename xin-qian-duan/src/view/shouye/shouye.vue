    ...
    <el-container style="height: 100%" direction="vertical">
      <el-header style="font-size: 12px;text-align: left">
        <span style="font-size:25px;margin-right:60%;">
          ...
          <!--提示音-->
          <audio id="tishiyin" src="../../../static/tishiyin/tishiyin.mp3"></audio>
          <audio id="happy" src="../../../static/tishiyin/happy.mp3"></audio>
          <audio id="en" src="../../../static/tishiyin/shizi.mp3"></audio>
          <audio id="qingsong" src="../../../static/tishi/qingsong.mp3"> </audio>
          <audio id="aiya" src="../../../static/tishi/aiya.mp3"> </audio>
          <audio id="shizi" src="../../../static/tishiyin/shizi.mp3"></audio>
...
<script>
  ...
  export default {
    ...
    name: "shouye",
    methods:{
      webSocketInit(){//提示音 //webSocket初始化，得判断浏览器是否支持WebSocket
           this.$data.webSocket= new WebSocket("ws://localhost:8888/webSocketTest/sendMessage/10");
           ...
           this.$data.onerror=this.errorWebSocket;

      },
      openWebSocket(event){ //打开通道
        this.playAudio("en","en")
        this.$data.webSocket.send("hello, 注意，注意 ，前端！！！")
      },
      messageWebSocket(event){//接受消息
        this.playAudio("qingsong","qingsong")
        this.$alert("该考试了","考试提醒",{
          type:'waring',
    ...
    mounted(){
      //登录之后提示
      this.playAudio("shizi","shizi");
      this.webSocketInit();//初始化提示音
